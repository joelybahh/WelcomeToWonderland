...
public class WW_RopeParticleSystem : MonoBehaviour {

    #region Private Variables

    private List<WW_RopeParticle> m_particles = new List<WW_RopeParticle>();
    ...
    [SerializeField] private Transform m_prefabParticle;
    [SerializeField] private Transform m_cableStart;
    [SerializeField] private Transform m_cableEnd;   
    [SerializeField] private int m_cableLength       = 10;
    [SerializeField] private int m_numSegments       = 10;
    ...
    void Start() {
        ...
        for (int ParticleIndex = 0; ParticleIndex < NumParticles; ParticleIndex++) {
            ...
            WW_RopeParticle particle = newTransform.GetComponent<WW_RopeParticle>();
    ...
    void Update() {
        ...
        WW_RopeParticle StartParticle = m_particles[0];
        StartParticle.position = StartParticle.m_oldPos = m_cableStart.position;

        WW_RopeParticle EndParticle = m_particles[m_numSegments];
    ...
    private void VerletIntegrate(float a_inSubstepTime, Vector3 a_gravity) {
        ...
        for (int ParticleIndex = 0; ParticleIndex < NumParticles; ParticleIndex++) {
            WW_RopeParticle particle = m_particles[ParticleIndex];
    ...
    private void SolveConstraints() {
        ...
        for (int IterationIndex = 0; IterationIndex < m_solverIterations; IterationIndex++) {
            ...
            for (int SegmentIndex = 0; SegmentIndex < m_numSegments; SegmentIndex++) {
                WW_RopeParticle ParticleA = m_particles[SegmentIndex];
                WW_RopeParticle ParticleB = m_particles[SegmentIndex + 1];
    ...
    void SolveDistanceConstraint(WW_RopeParticle a_particleA, WW_RopeParticle a_particleB, float a_desiredDistance) {
